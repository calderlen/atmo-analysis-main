...
def combine_ccfs_asymmetry(drv, cross_cor, sigma_cross_cor, orbital_phase, n_spectra, ccf_weights, half_duration_phase, temperature_profile, phase_ranges):
    ...
    idx_2 = np.where(np.isin(orbital_phase, orbital_phase_2))[0]
    ...
    for Kp_i in Kp:
        ...
        for j in range(len(orbital_phase_1)):
                ...
                var_shifted_ccfs_1[i,:] += sigma_temp_ccf_1**2 * ccf_weights[j]**2
    ...
    sigma_shifted_ccfs_1 = np.sqrt(var_shifted_ccfs_1)
    sigma_shifted_ccfs_2 = np.sqrt(var_shifted_ccfs_2)
                                   
    shifted_ccfs_1 -= np.median(shifted_ccfs_1) #handle any offset
    shifted_ccfs_2 -= np.median(shifted_ccfs_2) #handle any offset
...
def gaussian_fit_asymmetry(Kp, Kp_true, drv, species_label, planet_name, observation_epoch, arm, species_name_ccf, model_tag, snr, plotsnr_1, plotsnr_2, sigma_shifted_ccfs, sigma_shifted_ccfs_1, sigma_shifted_ccfs_2, phase_ranges):
    ...
    rv_1, rv_error_1 = np.zeros(plotsnr_1.shape[0]), np.zeros(plotsnr_1.shape[0])
    width_1, width_error_1 = np.zeros(plotsnr_1.shape[0]), np.zeros(plotsnr_1.shape[0])
    fwhm_1, fwhm_error_1 = np.zeros(plotsnr_1.shape[0]), np.zeros(plotsnr_1.shape[0])

    amps_2, amps_error_2 = np.zeros(plotsnr_2.shape[0]), np.zeros(plotsnr_2.shape[0])
    rv_2, rv_error_2 = np.zeros(plotsnr_2.shape[0]), np.zeros(plotsnr_2.shape[0])
    width_2, width_error_2 = np.zeros(plotsnr_2.shape[0]), np.zeros(plotsnr_2.shape[0])
    fwhm_2, fwhm_error_2 = np.zeros(plotsnr_2.shape[0]), np.zeros(plotsnr_2.shape[0])
    
    amps, amps_error = np.zeros(snr.shape[0]), np.zeros(snr.shape[0])
    rv, rv_error = np.zeros(snr.shape[0]), np.zeros(snr.shape[0])
    width, width_error = np.zeros(snr.shape[0]), np.zeros(snr.shape[0])
    fwhm, fwhm_error = np.zeros(snr.shape[0]), np.zeros(snr.shape[0]) 
    ...
    for i in Kp_valid:
        ...
        # If a valid peak was found, fit the Gaussian
        if valid_peaks[peak_1]:
            slice_peak_1[i] = plotsnr_1[i, peak_1]
            
            popt_1, pcov_1 = curve_fit(gaussian, drv, plotsnr_1[i,:], p0=[plotsnr_1[i, peak_1], drv[peak_1], 2.55035], sigma = sigma_shifted_ccfs_1[i,:], maxfev=1000000)
    ...
    for i in Kp_valid:
        ...
        # If a valid peak was found, fit the Gaussian
        if valid_peaks[peak_2]:
            slice_peak_2[i] = plotsnr_2[i, peak_2]
            
            popt_2, pcov_2 = curve_fit(gaussian, drv, plotsnr_2[i,:], p0=[plotsnr_2[i, peak_2], drv[peak_2], 2.55035], sigma = sigma_shifted_ccfs_2[i,:], maxfev=1000000)

            amps_2[i] = popt_2[0]
            rv_2[i] = popt_2[1]
            width_2[i] = popt_2[2]
            amps_error_2[i] = np.sqrt(pcov_2[0,0])
            rv_error_2[i] = np.sqrt(pcov_2[1,1])
            width_error_2[i] = np.sqrt(pcov_2[2,2])
    ...
    for i in Kp_valid:
        # Get the index of the peak with the maximum value within the desired range
        valid_peaks = np.abs(drv) <= 15
        peak = np.argmax(snr[i, :] * valid_peaks)

        # If a valid peak was found, fit the Gaussian
        if valid_peaks[peak]:
            slice_peak[i] = snr[i, peak]
            
            popt, pcov = curve_fit(gaussian, drv, snr[i,:], p0=[snr[i, peak], drv[peak], 2.55035], sigma = sigma_shifted_ccfs[i,:], maxfev=1000000)
    ...
    idx = np.argmax(slice_peak) #Kp slice corresponding to max SNR
    ax1.plot(drv, snr[idx, :], color='black', label='Full transit', alpha=0.8)
    ax1.text(0.95, 0.85, f'Full transit $K_p$ slice: {Kp[idx]}', transform=ax1.transAxes, verticalalignment='top', horizontalalignment='right', fontsize=10)
    ...
    #    RV = Kp_i*np.sin(2.*np.pi*orbital_phase)
    #    for j in idx_1:
    #        temp_ccf_1 = np.interp(drv, drv-RV[j], cross_cor[j, :], left=0., right=0.0)
    #        temp_ccf_1 *= ccf_weights[j]
    #        peak_1 = np.argmax(temp_ccf_1[390:411]) + 390
    #        sigma_temp_ccf_1 = np.interp(drv, drv-RV[j], sigma_cross_cor[j, :], left=0., right=0.0)
    #        sigma_temp_ccf_1 = sigma_temp_ccf_1**2 * ccf_weights[j]**2
    #        popt_1, pcov_1 = curve_fit(gaussian, drv, temp_ccf_1, p0=[temp_ccf_1[peak_1], drv[peak_1], 2.5], sigma = np.sqrt(sigma_temp_ccf_1), #maxfev=1000000)
    #        rv_chars_1[i,phase_here] = popt[1]
    #        rv_chars_error_1[i,phase_here] = np.sqrt(pcov_1[1,1])
    #        slice_peak_chars_1[i] = temp_ccf_1[peak]
    #
    #    for j in idx_2:
    #        temp_ccf_2 = np.interp(drv, drv-RV[j], cross_cor[j, :], left=0., right=0.0)
    #        temp_ccf_2 *= ccf_weights[j]
    #        peak_2 = np.argmax(temp_ccf[390:411]) + 390
    #        sigma_temp_ccf_2 = np.interp(drv, drv-RV[j], sigma_cross_cor[j, :], left=0., right=0.0)
    #        sigma_temp_ccf_2 = sigma_temp_ccf_2**2 * ccf_weights[j]**2
    #        popt_2, pcov_2 = curve_fit(gaussian, drv, temp_ccf_2, p0=[temp_ccf_2[peak_2], drv[peak_2], 2.5], sigma = np.sqrt(sigma_temp_ccf_2), #maxfev=1000000)
    #        rv_chars_2[i,phase_here] = popt[1]
    #        rv_chars_error_2[i,phase_here] = np.sqrt(pcov_2[1,1])
    #        slice_peak_chars_2[i] = temp_ccf_2[peak]
...
def make_shifted_plot_asymmetry(snr, snr_1, snr_2, planet_name, observation_epoch, arm, species_name_ccf, model_tag, RV_abs, Kp_expected, V_sys_true, Kp_true, do_inject_model, drv, Kp, species_label, temperature_profile, sigma_shifted_ccfs, sigma_shifted_ccfs_1, sigma_shifted_ccfs_2, method, cross_cor_display, sigma_cross_cor, ccf_weights, phase_ranges, plotformat = 'pdf'):
    ...
    plotsnr_1, plotsnr_2, drv = plotsnr_1[:, keeprv], plotsnr_2[:, keeprv], drv[keeprv]
    #keepKp = np.abs(Kp-apoints[1]) <= 100.
    keepKp = np.abs(Kp-apoints[1]) <= 401.
    ...
    # Fit a Gaussian to the line profile
    gaussian_fit_asymmetry(Kp, Kp_true, drv, species_label, planet_name, observation_epoch, arm, species_name_ccf, model_tag, snr, plotsnr_1, plotsnr_2, sigma_shifted_ccfs, sigma_shifted_ccfs_1, sigma_shifted_ccfs_2, phase_ranges)
